...
namespace Redsz.Services
{
    ...
    public class SystemServices
    {
        ...
        public string ContextExecutive(HttpContext context)
        {
            ...
            try
            {
                ...
                foreach (MethodInfo info in methodInfo)
                {
                    if (info.Name.Equals(methodName))
                    {
                        ...
                        {
                            returnString = "{\"success\":false,\"message\":\"" + reqMethod + "(HttpContext context) 执行异常：<br/>"+e.Message.Replace("\r","<br/>").Replace("\n", "<br/>").Replace("\"", "") + "\"}";
                        }
                        break;
                    }
                }
                ...
                {
                    returnString = "{\"success\":false,\"message\":\""+reqMethod + "(HttpContext context) 不存在！\"}";
                }

            }
            catch 
            {
                returnString = "{\"success\":false,\"message\":\"对象 "+ className + " 不存在！\"}";
            }
            return returnString;
        }
        ...
        /// </summary> 
        /// <returns>Object </returns> 
        public string ContextExecutiveDebug(HttpContext context)
        {
            HttpRequest req = context.Request;
            string returnString = "";
            string reqMethod = req["method"];
